...
import { getBloodBanks } from 'apiSdk/blood-banks';
import { BloodBankInterface } from 'interfaces/blood-bank';
import { getBloodRequests } from 'apiSdk/blood-requests';
...
import { BloodRequestListPage as ShowCaseList } from 'pages/blood-requests';

function DashboardPage() {
  const { session } = useSession();

  const bloodBankStat = useSWR<PaginatedInterface<BloodBankInterface>>(
    () => `/blood-banks`,
    () => getBloodBanks({ limit: 1, offset: 0 }),
  );

  const bloodRequestStat = useSWR<PaginatedInterface<BloodRequestInterface>>(
    () => `/blood-requests`,
    () => getBloodRequests({ limit: 1, offset: 0 }),
  );

  const staffMemberStat = useSWR<PaginatedInterface<StaffMemberInterface>>(
    () => `/staff-members`,
    () => getStaffMembers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Blood Banks',
      result: bloodBankStat?.data?.totalCount,
      isLoading: bloodBankStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Blood Requests',
      result: bloodRequestStat?.data?.totalCount,
      isLoading: bloodRequestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Staff Members',
      result: staffMemberStat?.data?.totalCount,
      isLoading: staffMemberStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`blood-requests`} passHref legacyBehavior>
                  <Button
