...
import { getBloodBanks, deleteBloodBankById } from 'apiSdk/blood-banks';
import { BloodBankInterface } from 'interfaces/blood-bank';

type ColumnType = ColumnDef<BloodBankInterface, unknown>;
...
export function BloodBankListPage(props: BloodBankListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getBloodBanks({
        ...
        order: params.order,
        searchTermKeys: ['blood_type.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BloodBankInterface>>(
    () => `/blood-banks?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BloodBankInterface) => {
    if (hasAccess('blood_bank', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/blood-banks/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/blood-banks/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('blood_bank', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/blood-banks/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('blood_bank', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="blood_bank" />
        </Flex>

        {hasAccess('blood_bank', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/blood-banks/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
